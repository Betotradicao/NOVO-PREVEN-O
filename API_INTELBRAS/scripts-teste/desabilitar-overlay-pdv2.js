/**
 * DESABILITAR OVERLAY PDV2 - FORÇA BRUTA
 */
...
async function main() {
  console.log('\n🔧 DESABILITANDO OVERLAY DO PDV2...\n');

  const idx = 5;
  const path = `/cgi-bin/configManager.cgi?action=setConfig&PosConfig[${idx}].OverlayEnable=false`;
  ...
  try {
    ...
    if (response.body.includes('OK') || response.body.includes('ok')) {
      console.log('✅ Overlay DESABILITADO com sucesso!\n');
    } else {
